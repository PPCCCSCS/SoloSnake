...
class Snek:
    ...
    def addCard(self,card):
        ...
        if index < len(self.queue)-1:
            ...
            card.nextMove = self.queue[index].nextMove
            self.queue[index].isTop = False
...
class Card:
    ...
    def move(self):
        ...
        if update:
            self.tile = [int(self.pos[0]/TS), int(self.pos[1]/TS)]
...
def main():
    ...
    while not gameover:
        ...
        if sn.queue[0].thisMove != [0,0]:
            thisTile = gb.tileContains(hp)        
            if type(thisTile) is Card:
                #print([thisTile])
                thisTile.Pos = tp
                sn.addCard(thisTile)
                gb.field[hp[0]][hp[1]] = ""
        ...
        #
        #
        for square in sn.queue[2:]:
            # get the tile position of this square
            tpos = [int(square.pos[0]/TS), int(square.pos[1]/TS)]
            # check what this tile position contains
            temp = gb.tileContains(tpos)

            tspaces.append(tpos)
          
            # Check if card to avoid overwriting cards
            if type(temp) is Card:

                if sn.addCard(temp):
                    gb.dealtCards.remove(temp)
                    gb.field[tpos[0]][tpos[1]] = ""
            
        if STARTED == True and hp in tspaces:
            gameover = True
        else:
            tspaces.clear()

        #print(sn.queue[0].pos,START)
        if [int(sn.queue[0].pos[0]/TS),int(sn.queue[0].pos[1]/TS)] != START:
            STARTED = True
